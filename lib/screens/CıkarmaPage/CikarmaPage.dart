...
import 'package:dort_islem/screens/C%C4%B1karmaPage/CikarmaPageBody.dart';
...
class CikarmaPage extends StatefulWidget {
  const CikarmaPage({super.key});

  @override
  State<CikarmaPage> createState() => _CikarmaPageState();
}
...
class _CikarmaPageState extends State<CikarmaPage> {
  ...
  Future<List<CikarmaModel>> getData() async {
    return db.fetchSubQuestion();
  }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: _questions as Future<List<CikarmaModel>>,
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.done) {
          if (snapshot.hasError) {
            ...
            var extractData = snapshot.data as List<CikarmaModel>;
            return Scaffold(
